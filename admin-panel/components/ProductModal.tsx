...
export function ProductModal({ categoryId, locales, product, onClose, onSaved }: Props) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      const body = { name, description: Object.keys(description).length ? description : undefined, price, photoUrl: photoUrl || undefined, sortOrder };
          ...
          <div>
            ...
            {photoUrl && (
              <div className="mb-2 relative w-24 h-24">
                ...
                <button type="button" onClick={() => setPhotoUrl('')} className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full w-5 h-5 text-xs flex items-center justify-center">×</button>
              </div>
            )}
          ...
          <div className="flex gap-2 pt-2">
            ...
            {isEdit && (
              <button type="button" onClick={handleDelete} disabled={loading}
                className="px-4 bg-red-100 text-red-700 rounded-lg py-2 text-sm font-medium hover:bg-red-200">
                Delete
              </button>
            )}
