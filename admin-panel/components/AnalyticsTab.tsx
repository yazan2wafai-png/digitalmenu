    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        ...
        <div className="flex items-center space-x-4 text-sm">
          ...
          <button 
            ...
            disabled={loading}
            className="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-800 rounded disabled:opacity-50 transition-colors"
          >
      ...
      {data && (
        <>
          ...
          <div className="mt-8 bg-white p-4 rounded shadow border border-gray-100">
...
function StatCard({ title, value }: { title: string; value: number }) {
  return (
    <div className="bg-white p-4 rounded shadow border border-gray-100 flex flex-col">
      <span className="text-gray-500 text-sm font-medium">{title}</span>
      <span className="text-3xl font-bold mt-2">{value?.toLocaleString() || 0}</span>
    </div>
  );
}
