...
export default function AnalyticsTab({ slug }: { slug: string }) {
  ...
  if (loading && !data) {
    return <div className="p-6 text-gray-500">{t.analytics.loading}</div>;
  }

  if (error && !data) {
    return (
      <div className="p-6">
        <div className="text-red-500 mb-4">{t.analytics.error} {error}</div>
        <button onClick={fetchAnalytics} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 cursor-pointer">
          {t.analytics.retry}
        </button>
      </div>
    );
  ...
  return (
    <div className="p-6 space-y-6 text-gray-900">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <h2 className="text-xl font-bold text-gray-800">{t.analytics.title} <span className="text-blue-600">@{slug}</span></h2>
        <div className="flex items-center space-x-4 text-sm">
          {lastUpdated && <span className="text-gray-500 text-xs">{t.analytics.lastUpdated} {lastUpdated.toLocaleTimeString()}</span>}
          <button 
            onClick={fetchAnalytics} 
            disabled={loading}
            className="px-3.5 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-800 text-xs font-semibold rounded-lg disabled:opacity-50 transition-colors cursor-pointer"
          >
            {loading ? t.analytics.refreshingBtn : t.analytics.refreshBtn}
          </button>
        </div>
      </div>
      ...
      {data && (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard title={t.analytics.totalViews} value={data.totalViews} />
            <StatCard title={t.analytics.todayViews} value={data.todayViews} />
            <StatCard title={t.analytics.last7Days} value={data.last7DaysViews} />
            <StatCard title={t.analytics.last30Days} value={data.last30DaysViews} />
          </div>

          <div className="mt-8 bg-white p-6 rounded-xl shadow-xs border border-gray-200">
            <h3 className="text-lg font-semibold mb-4 text-gray-800">{t.analytics.trendTitle}</h3>
            <TrendChart data={data.dailyBreakdown} noDataText={t.analytics.noData} />
          </div>
        </>
      )}
    </div>
...
function TrendChart({ data, noDataText }: { data: DailyBreakdown[]; noDataText: string }) {
  if (!data || data.length === 0) return <div className="text-gray-500 py-8 text-center">{noDataText}</div>;
