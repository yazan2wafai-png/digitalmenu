...
export function NfcTapSimulator() {
  ...
  const TABS: { id: Screen; label: string; icon: string; accentColor: string }[] = [
    { id: 'google', label: 'Google Yorum Kartı', icon: '⭐', accentColor: '#C9A86C' },
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 space-y-4">
          ...
          >
            ...
            Canlı NFC Tap & Menü Simülatörü
          </div>
          <h2 className="text-4xl sm:text-5xl font-black tracking-tight" style={{ color: '#F0E6D3' }}>
            Karta Dokunun, <span className="gold-text">Telefon Ekranında Deneyimleyin.</span>
          </h2>
          <p className="text-base" style={{ color: 'rgba(180,152,104,0.75)' }}>
            NFC kart veya menü çipinin telefonda 0.2 saniyede nasıl açıldığını test edin.
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-2.5 pt-2">
            {TABS.map((tab) => (
              <button
                ...
                style={{
                  background:
                    screen === tab.id && tapped
                      ? `rgba(201,168,108,0.18)`
                      : 'rgba(18,14,8,0.8)',
                  border: `1px solid ${
                    screen === tab.id && tapped ? 'rgba(201,168,108,0.6)' : 'rgba(201,168,108,0.12)'
                  }`,
                  color: screen === tab.id && tapped ? '#F0E6D3' : 'rgba(180,152,104,0.7)',
                }}
              >
                {tab.icon} {tab.label}
              </button>
        ...
        >
          ...
          <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            {/* Left: Physical Acrylic NFC Card / Stand */}
            <div className="flex flex-col items-center gap-6">
              <div className="relative flex items-center justify-center">
                {/* Expanding magnetic wave rings */}
                {[1, 2, 3].map((r) => (
                  <motion.div
                    key={r}
                    className="absolute rounded-full pointer-events-none"
                    style={{ width: r * 115, height: r * 115, border: '1px solid rgba(201,168,108,0.18)' }}
                    animate={{ scale: [1, 1.06, 1], opacity: [0.3, 0.55, 0.3] }}
                    transition={{ duration: 3.2, repeat: Infinity, delay: r * 0.6, ease: 'easeInOut' }}
                  />
                ))}
                ...
                <motion.div
                  ...
                  whileTap={{ scale: 0.97 }}
                  onClick={() => tap(screen)}
                  className="cursor-pointer relative w-72 h-44 rounded-3xl flex flex-col justify-between p-5 select-none z-10 transition-shadow"
                  style={{
                ...
                >
                  ...
                  <div className="text-center space-y-1">
                    ...
                    <span className="text-xs font-bold block" style={{ color: 'rgba(240,230,211,0.95)' }}>
                      ...
                      {screen === 'baltazar' && 'Baltazar Burger Menü Çipi'}
                      {screen === 'erenkoy' && 'Kahve Erenköy Menü Çipi'}
                      {screen === 'actnoir' && 'Act Noir Café Menü Çipi'}
                    </span>
                    <span className="text-[10px] font-semibold block" style={{ color: 'rgba(201,168,108,0.65)' }}>
                      Karta Dokunarak Telefonu Açın
                    </span>
                  </div>
                  <div className="flex justify-between font-mono text-[9px]" style={{ color: 'rgba(201,168,108,0.4)' }}>
                    <span>NTAG213 / 215</span>
                    <span>TEMASSIZ DOKUNUŞ</span>
                  </div>
                </motion.div>
              </div>

              <p className="text-xs font-semibold flex items-center gap-1.5 text-center" style={{ color: 'rgba(201,168,108,0.7)' }}>
                <span>👆 Karta tıklayın veya yukarıdaki menü butonlarına basın</span>
              </p>
            </div>
            ...
            <div className="flex justify-center">
              <div
                className="relative w-[300px] h-[560px] rounded-[44px] p-4 flex flex-col"
                style={{
                  background: '#0A0806',
                  ...
                  {!tapped ? (
                    <div className="m-auto text-center space-y-4" style={{ color: 'rgba(180,152,104,0.5)' }}>
                      ...
                      <p className="text-xs px-6 leading-relaxed">
                        Soldaki karta tıklayın veya yukarıdan bir restoran seçin; telefon arayüzü anında canlanacaktır.
                      </p>
                    </div>
                  ) : (
                    <AnimatePresence mode="wait">
                        ...
                        >
                          <div
                            ...
                            style={{
                              ...
                              border: '1px solid rgba(201,96,74,0.35)',
                            }}
                          >
                            <div>
                              <span className="text-xs font-black block" style={{ color: '#F0E6D3' }}>
                                Baltazar Burger
                              </span>
                              <span className="text-[10px]" style={{ color: '#C9604A' }}>
                                Masa 12 • Karaköy
                              </span>
                            </div>
                            <span
                              className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                              style={{ background: 'rgba(201,96,74,0.2)', color: '#C9604A' }}
                            >
                              Canlı Menü
                            </span>
                          </div>
                          {[
                            { n: 'Truffle Smash Burger', d: 'Karamelize Soğan, Trüf Mayonez', p: '340 TL' },
                            { n: 'Cheddar Loaded Fries', d: 'Eritilmiş Cheddar, Jalapeño', p: '180 TL' },
                          ].map((item) => (
                            <div
                              key={item.n}
                            ...
                            >
                              <div>
                                ...
                                <span className="text-[11px] font-bold block mt-0.5" style={{ color: '#C9A86C' }}>
                                  {item.p}
                                </span>
                              </div>
                              <button
                          ...
                          <button
                            onClick={() => alert(`Masa 12 için sipariş mutfağa iletildi! (${cart} adet)`)}
                            className="w-full py-2.5 rounded-xl text-xs font-bold cursor-pointer flex items-center justify-center gap-1.5"
                            style={{ background: 'linear-gradient(135deg, #C9604A, #8A3020)', color: 'white' }}
                          >
                            <ShoppingBag className="w-3.5 h-3.5" /> Masadan Siparişi Ver {cart > 0 && `(${cart})`}
                          </button>
                        </motion.div>
                      )}
                      ...
                      {screen === 'erenkoy' && (
                        ...
                        >
                          ...
                          >
                            <div>
                              ...
                              <span className="text-[10px]" style={{ color: gold }}>
                                Masa 4 • Nitelikli V60 Barı
                              </span>
                            </div>
                          </div>
                          {[
                            { n: 'V60 Ethiopia Yirgacheffe', d: 'Çiçeksi, Narenciye Reçetesi', p: '170 TL' },
                            { n: 'San Sebastian Cheesecake', d: 'Karamelize Kabuk & Akışkan', p: '160 TL' },
                          ].map((item) => (
                            <div
                              key={item.n}
                              className="p-2.5 rounded-xl flex items-center justify-between"
                            ...
                            >
                              <div>
                                ...
                                <span className="text-[11px] font-bold block mt-0.5" style={{ color: gold }}>
                                  {item.p}
                                </span>
                              </div>
                              <button
                          ...
                          <button
                            onClick={() => alert(`Kahve Erenköy siparişi iletildi!`)}
                            className="w-full py-2.5 rounded-xl text-xs font-bold cursor-pointer flex items-center justify-center gap-1.5"
                            style={{ background: 'linear-gradient(135deg, #C9A86C, #8A6835)', color: '#0D0B08' }}
                          >
                            <ShoppingBag className="w-3.5 h-3.5" /> Siparişi İlet {cart > 0 && `(${cart})`}
                          </button>
                        </motion.div>
                      )}
                        ...
                        >
                          <div
                            ...
                            style={{
                              ...
                              border: '1px solid rgba(139,156,176,0.35)',
                            }}
                          >
                            <div>
                              <span className="text-xs font-black block" style={{ color: '#F0E6D3' }}>
                              ...
                              <span className="text-[10px]" style={{ color: '#8B9CB0' }}>
                                Moda • Butik Fırın & Kahve
                              </span>
                            </div>
                          </div>
                          {[
                            { n: 'Flat White Double Shot', d: 'Kadifemsi Mikro Köpük', p: '140 TL' },
                            { n: 'Bademli Kruvasan', d: 'Fransız Tereyağlı Taze', p: '110 TL' },
                          ].map((item) => (
                            <div
                              key={item.n}
                              className="p-2.5 rounded-xl flex items-center justify-between"
                              style={{
                                ...
                                border: '1px solid rgba(139,156,176,0.12)',
                              }}
                            >
                              <div>
                                <span className="text-xs font-bold block" style={{ color: '#F0E6D3' }}>
                                ...
                                <span className="text-[11px] font-bold block mt-0.5" style={{ color: '#8B9CB0' }}>
                                  {item.p}
                                </span>
                              </div>
                              <button
                          ...
                          <button
                            onClick={() => alert(`Act Noir siparişi iletildi!`)}
                            className="w-full py-2.5 rounded-xl text-xs font-bold cursor-pointer flex items-center justify-center gap-1.5"
